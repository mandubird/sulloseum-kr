...
export type BoardAgentMap = Record<string, { persona_name: string; avatar_emoji: string }>
...
export default function BoardClient({
  ...
  initialBattles?: BattleItem[]
  initialAgentMap?: BoardAgentMap
}) {
  const router = useRouter()
  const [battles, setBattles] = useState<BattleItem[]>(initialBattles)
  const [agentMap, setAgentMap] = useState<BoardAgentMap>(initialAgentMap)
  const [loading, setLoading] = useState(initialBattles.length === 0)
